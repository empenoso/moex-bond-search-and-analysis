...
def measure_method_duration(foo: Callable) -> Callable:
    def wrapper(self, *args, **kwargs):
        ...
        self.log.info(f"🚀 Функция {foo.__name__} начала работу в {datetime.now().strftime(DATETIME_FORMAT)}.")
        result = foo(self, *args, **kwargs)
        duration = humanize.precisedelta(int(time.monotonic()) - start_time, minimum_unit="seconds", format="%0.0f")
        self.log.info(f"✅ Функция {foo.__name__} закончила работу в {datetime.now().strftime(DATETIME_FORMAT)}.") 
